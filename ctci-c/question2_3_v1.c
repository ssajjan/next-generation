...
int main(int argc, char *argv[]) {
	...
	*/
	...
	midNode = findNthNode(head,mid);
	if (midNode)
		printf("Value: %d, Address: %p, Next Address: %p\n",midNode->data,midNode,midNode->next);
	else
		printf("No %dth node present in the linked list\n",mid);

	bool status;
	status = delNthNode(midNode);
	if (status) 
		printf("%dth node deleted\n",mid);
	else
		printf("%dth node cant be deleted\n",mid);
