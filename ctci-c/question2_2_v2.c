...
struct node * findKtoLast(struct node * start, int k) {
...
int main(int argc, char *argv[]) {
	...
	*/
	...
	nodektl = findKtoLast(head,k);

	if (nodektl) {
		printf("Value (last-%d) element of the list: %d at %p pointer\n",k,nodektl->data,nodektl);
	}
	else {
		printf("Value (last-%d) element of the list does not exist\n",k);
	}
