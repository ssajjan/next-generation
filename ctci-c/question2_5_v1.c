...
struct node * padList(struct node *start, int len, int reqlen) {
	struct node *curr = start;
	while (curr->next != NULL) {
		curr = curr->next;
	}
	
        for (int i=1; i <= reqlen-len; i++) {
                curr = appendNode(curr,0);
        }
        return start;
}

struct node * addLists(struct node *num1, struct node *num2) {
	struct node *sum = num1;
	struct node *savePrev1 = NULL;
	struct node *savePrev2 = NULL;
	struct node *saveNext = NULL;
	int carry = 0;
	while (num1 != NULL) {
		saveNext = num1->next;
	...
	if (carry == 0) {
		...
		savePrev1->next = savePrev2;
		savePrev2->data = carry;
		return sum;
	}
}
