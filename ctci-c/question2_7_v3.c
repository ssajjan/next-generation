...
bool isPalindromeRecurse(struct node *start, int len, struct node **Node) {
	...
	if (len == 1) {
		...
		*Node = start;
		...
		palinStatus = isPalindromeRecurse(start->next, len-2, Node);
		if (*Node) {
			printf("Start: %p, data: %d, Node: %p, data: %d\n",start,start->data,*Node,(*Node)->data);
...
bool isPalindrome(struct node *start) {
	...
	bool status;
	status = isPalindromeRecurse(start, findLen(start), &Node);
...
int main(int argc, char *argv[]) {
	...
	*/
	...
	int num[MAX_LEN] = {10,2,3,4,5,6,4,3,2,10};
