...
int getCarry (struct node *num1, struct node * num2) {
	int carry;
	int sumNode;

	if (num1->next != NULL) {
		carry = getCarry(num1->next,num2->next);
		sumNode = num1->data + num2->data + carry;
	}
	else {
		sumNode = num1->data + num2->data;
	}
...
struct node * addLists(struct node *num1, struct node *num2) {
	struct node *total = num1;
	struct node *saveNext = NULL;
	struct node *savePrev = NULL;
	int carry;
	while (num1 != NULL) {
		saveNext = num1->next;
		if (num1->next == NULL) 
