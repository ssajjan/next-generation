...
bool findiBit(unsigned int *A, int n, int bitindex) {
	int index = 0, count0s = 0, count1s = 0;
...
unsigned int findMissing(unsigned int *A, int n){
	int bitindex = 0;
	unsigned int num = 0;

	while (bitindex <= log2(n)) {
		num += findiBit(A,n,bitindex)*(pow(2,bitindex));
		printf("Num: %u\n",num);
		bitindex++;
	}
