...
char * dequeueDog(char *str) {
	struct node *temp = NULL, *curr= NULL, *prev = NULL;
	//static char str[MAX_LEN];           /*defining as a static increases local variable longevity*/
	...
	if (end == NULL && head == NULL) {
		...
		curr = head;
		while (curr != NULL && !strstr(curr->animal,"Dog")) {
			prev = curr;
			curr = curr->next;
		}

		if (prev == NULL) {
			//First animal is dog itself
			strcpy(str,head->animal);
			if (head != end) {
				temp = head;
				head = head->next;
			...
			if (curr) {
				...
				printf("No Dog left in the queue\n");
				return false;
			}
		}
	}
...
char * dequeueCat(char *str) {
	...
	if (end == NULL && head == NULL) {
		...
		curr = head;
		while (curr != NULL && !strstr(curr->animal,"Cat")) {
			prev = curr;
			curr = curr->next;
		}

		if (prev == NULL) {
			//First animal is Cat itself
			strcpy(str,head->animal);
			if (head != end) {
				temp = head;
				head = head->next;
			...
			if (curr) {
				...
				printf("No Cat left in the queue\n");
				return false;
			}
		}
	}
