...
void delDupNodesN(struct node *start) {
	struct node *current=start,*runner;

	/* Remove all future nodes that have the same value */
	while (current != NULL) {
		printf("Current pointer: %p, value: %d\n",current,current->data);
		runner = current;
		while (runner->next != NULL) {
			printf("Runner pointer: %p, value: %d\n",runner->next,runner->next->data);
...
int main(int argc, char *argv[]) {
	...
	*/
	int arr[len] = {5,5,5,5,5,0,2,7,2,15,6,6,0,15,5};
