...
int main(int argc, char *argv[]) {
	...
	*/
	...
	int arr[MAX_LEN] = {5,60,14,50,9,10,5,14,130,14,0,1,2,13,4};
	...
	int factor = (end-head)/(MAX_LEN-1);
	int elemk ;
	/* Find 5th to the end of list
		total elements = N
		5th to the last = end-5
		we have head
		N = int(head-end)+1
		int(head-end) = N-1
		head+N-1 = end
	hence   5th to the last = head+N-1-5
				= head+N-((5)+1)
	hence   kth to the last = head+N-(k+1)
	*/

	struct node *elemkPtr;

	printf("\n");
	for (elemk = 0; elemk<MAX_LEN;elemk++) {
		elemkPtr = head+factor*(MAX_LEN-(elemk+1));
		printf("Value at elem%dPtr: %p is: %d\n",elemk,elemkPtr,*elemkPtr);
	}
